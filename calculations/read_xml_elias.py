...
def check_none(element, exception):
    fixed_element = (
        element.text if element is not None and element.text is not None else exception
    )
...
def get_fast_avgift(root):
    ...
    alla_andelsklasser = find_all_elements(
        root, ["Förvaltningsavgift", "MedAndelsklasser", "Förvaltningsavgift"]
    )
    ...
    if alla_andelsklasser:
        ...
        for andelsklass in alla_andelsklasser:
            andelsklass_namn = (
                find_element(andelsklass, ["Andelsklass_namn"]).text or "Unknown Class"
            )
            avgift_fast = float(
                find_element(
                    andelsklass, ["Förvaltningsavgift_Typ", "Förvaltningsavgift_fast"]
                ).text
                or 0
            )
        ...
        avgift = float(
            find_element(alla_utan[0], ["Förvaltningsavgift_fast"]).text or 0
        )
...
def main():
    root_folder_path = "calculations/data"
    ...
    for dirpath, dirnames, filenames in os.walk(root_folder_path):
        for filename in filenames:
            ...
            if filename.endswith(".xml"):  # Process only XML files
                ...
                if översikt["fondformogenhet"] <= 0:
                    continue
                fond_status_element = find_element(
                    root, ["Fondinformation", "Fond_status"]
                )
                aktiv = check_aktiv(fond_status_element)
                if not aktiv:
                    print(
                        f"Skipping {översikt['fond_namn']}: Fund is inactive (Ej aktiv fond)."
                    )
                    continue
                ...
                mappning = pd.concat(
                    [
                        ...
                        pd.DataFrame(
                            {
                                "fond_namn": [översikt["fond_namn"]],
                                "isin": [översikt["fond_isin"]],
                            }
                        ),
                    ]
