...
def calc(nivå_dict, mappning, alla_fonder):
    ...
    for i in range(nivåer):
        ...
        for fond in fonder:
            if i == 0:
                ...
                if not isin.empty:
                    ...
                    temp_df = pd.DataFrame(
                        {
                            "instument_isin": [fond],
                            "instrument_namn": [fond],
                            "landkod_emittent": "-",
                            "andel_av_fond": nivå_dict[i][fond],
                            "markandsvarde_instrument": nivå_dict[i][fond],
                            "bransch": "-",
                            "nivå": 0,
                        }
                    )
            ...
            innehav_per_nivå = pd.concat(
                [innehav_per_nivå, innehav], axis=0
            ).reset_index(drop=True)
        aktier = innehav_per_nivå.loc[
            ~innehav_per_nivå["instrument_isin"].isin(alla_fonder.keys())
        ]
        alla_aktier = pd.concat([alla_aktier, aktier], axis=0).reset_index(drop=True)
        nivå2 = innehav_per_nivå.loc[
            innehav_per_nivå["instrument_isin"].isin(alla_fonder.keys())
        ][["instrument_isin", "andel_av_fond"]]
